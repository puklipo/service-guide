...
function getServiceType($serviceCode) {
    ...
    if (in_array($serviceCode, $visitingServices)) return 'visiting_service';
    if (in_array($serviceCode, $facilityServices)) return 'facility_service';
    if (in_array($serviceCode, $consultationServices)) return 'consultation_service';
    if (in_array($serviceCode, $supportServices)) return 'support_service';
        ...
        if (($handle = fopen($csvFile, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle)) !== false) {
                ...
                if ($serviceCode !== null) {
                    ...
                    if ($capacity > 0) {
                        $sizeCategory = 'small';
                        if ($capacity >= 30) $sizeCategory = 'large';
                        elseif ($capacity >= 11) $sizeCategory = 'medium';
