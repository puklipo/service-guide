...
```blade
<x-chart.bar
  :data="[[6.6, 10.0], [5.0, 10.3], [5.0, 9.6], [6.7, 8.9], [5.8, 9.8]]"
  :labels="['児童発達支援', '放課後等デイ', '障害児相談支援', '保育所等訪問支援', '居宅訪問型児童発達支援']"
  title="児童系サービスの成長率変化（青：2025年3月、灰：2024年9月）"
/>
```
...
```blade
<x-chart.bar
  :data="[3.1, 2.3, 1.8, 9.1]"
  :labels="['就労移行支援', '就労継続支援A型', '就労継続支援B型', '就労定着支援']"
  title="就労支援系サービスの成長率（2025年3月、%）"
/>
```
